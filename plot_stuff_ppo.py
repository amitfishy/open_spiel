...
parser.add_argument('--primary_plot_feature', help='Parallel plots of this feature are made in the same graph. Options: [init_lrs] [ppo_epss]', default=None)
...
N_epochs = get_param('N_epochs', lines)
min_policy_eps = get_param('min_policy_eps', lines)
...
ppo_epss = get_list_params('ppo_epss', lines)



sec_plot_feat_init_lrs = (len(init_lrs) > 1) and (args.primary_plot_feature != 'init_lrs')
sec_plot_feat_ppo_epss = (len(ppo_epss) > 1) and (args.primary_plot_feature != 'ppo_epss')
...
for game_name in game_names:
	for init_lr in init_lrs:
		for actorcritic_method in actorcritic_methods:
			for ppo_eps in ppo_epss:
				results_args_list = [game_name, num_steps, print_freq, init_lr, regularizer_scale, num_hidden, num_layers, actorcritic_method, N_epochs, ppo_eps, min_policy_eps]
				...
				output_args_list = [game_name, actorcritic_method]
				if sec_plot_feat_init_lrs:
					output_args_list.append('LR-' + init_lr)
				if sec_plot_feat_ppo_epss:
					output_args_list.append('PPO-EPS-' + ppo_eps)
				...
				elif args.primary_plot_feature == 'ppo_epss':
					output_filename = 'PPOε-' + ppo_eps + '.npz'
